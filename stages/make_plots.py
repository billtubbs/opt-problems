...
def make_f_histogram(stats, exp_name, n_bins=51):
...
def make_plots(exp_name, exp_params):
    ...
    stats = pd.read_csv(
        os.path.join(
            RESULTS_DIR, exp_name, PREV_STAGE_RESULTS_DIR, "stats.csv"
        )
    )
    ax = make_f_histogram(stats, exp_name)
    plt.tight_layout()
    ...
    prev_stage_results_dir = os.path.join(
        RESULTS_DIR, exp_name, PREV_STAGE_RESULTS_DIR
    )
    filenames = sorted(
...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(RESULTS_DIR, exp_name, 'plots'), exist_ok=True)
